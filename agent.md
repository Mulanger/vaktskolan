...
Google Ads-köpsignalen verifieras separat av `GET /api/stripe/status?session_id=...`. Endpointen kräver Clerk-session och kontrollerar direkt mot Stripe att Checkout-sessionen tillhör samma användare, är betald, körs i rätt test/live-läge och innehåller exakt den konfigurerade Premium-produkten på 399 SEK. Frontend skickar därefter Ads-konverteringen med Stripe-sessionens id som `transaction_id` och gör korta bakgrundsförsök vid väntande betalning eller tillfälliga nätfel. Denna väg rapporterar endast mätdata och får aldrig ge Premium; signerad webhook förblir source of truth för åtkomst.
...
## Arbetsgivare-flik i Karriärverktyget (2026-07-24)
...
- Sökrutan filtrerar korten live på namn + adress via `filterEmployerDirectory()` (DOM-filtrering på `data-employer-search`, ingen omrendering så fokus behålls). Tom träfflista visar `.employers-empty`. En separat `input`-lyssnare på `knowledgeBasePanel` fångar `[data-employer-search-input]`.
- Stilen ligger i `styles.css` under `.employers*`/`.employer-*`, direkt före `.salary-check`. Grid: 3 kolumner desktop, 2 kolumner ≤1024px, 1 kolumn ≤760px. Konventioner (blå #2563eb, slate-paletten, kortradie, hover-lyft) speglar Schemaguide. Verifierat i browser på 1320px (3 kol), ~792px (2 kol) och 375px (1 kol, ingen horisontell overflow); sök och tomt-läge testade. Verifieringen gjordes via en isolerad förhandsvisning eftersom legacy-dashboarden kräver inloggad Clerk-användare som inte kan initieras lokalt.
- Cache: `styles.css`/`app.js`-query strings i `index.html` bumpade till `?v=20260724-employers-links`.

## Karriärverktyget på mobil (2026-07-24)

Karriärverktygets fyra flikar (Lönekollen, CV-mall, Schemaguide, Arbetsgivare) nåddes tidigare bara via desktop-sidebaren (`data-open-knowledge-base`); mobilens bottenmeny (`*-mobile-tabbar`) rymmer bara Hem/VU1/VU2/Quiz/Slutprov. Nu finns en mobil-ingång:

- En **verktygsknapp** (`.mobile-tools-button`, lucide `briefcase`, `data-open-career-menu`) ligger till vänster om profilavataren i de tre mobil-headrarna (Hem, Quizportalen, Slutprovsportalen), grupperad i `.mobile-head-actions`. Knappen ärver headrarnas mobil-synlighet (≤940px).
- Knappen öppnar **Karriärverktygsmenyn** – en bottensheet (`#careerMenu` i `index.html`) som listar de fyra verktygen. Den återanvänder modal-mönstret (`hidden`-attribut + `syncModalOpenState()` + `body.modal-open`), stängs via bakgrundsklick/`[data-close-career-menu]`/Escape. `openCareerMenu()`/`closeCareerMenu()` i `app.js`.
